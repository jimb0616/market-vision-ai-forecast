...
const StockChart = ({ 
  ...
  const predictionStartIndex = data.findIndex(point => point.price === undefined && point.prediction !== undefined);
  
  // Today's date (last real data point)
  const today = predictionStartIndex > 0 ? data[predictionStartIndex - 1].date : data[data.length - 1].date;
